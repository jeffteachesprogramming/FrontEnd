//  This TypeScript interface replaces the object literal we had
//  in the JavaScript version of our calculator.
//
//  Among other things, a TypeScript interface does the following:
//
//  1.  Provides a compile time error if a property name is misspelled.
//  2.  Provides clearer documentation of exactly what the state contains
//
//  The "|" used below is the TypeScript Union type which allows either
//  of the types shown (string or null) as valid types.
interface CalcState
{
    current:        string;             //  What is shown in the result display
    previous:       string | null;      //  The left hand operand or null if not set
    operator:       string | null;      //  +, -, *, / or null if not set
    waitForNext:    boolean;            //  True after an operator is pressed
    history:        string[];           //  Array of past expressions
}

//  Define a type alias for valid operator strings
type Operator = "add" | "subtract" | "multiply" | "divide";

//  Define a type for our keyboard map entries
...
//  currently active
function highlightOperator(op: string | null): void
{
    ...
    document.querySelectorAll(".btnOperation").forEach((btn: Element): void =>
    {
...
function resetState(seedValue?: string): void
{
...
function inputIsDigit(digit: string): void
{
...
//  the error display
function calculate(a: number, b: number, op: string | null): number
{
    switch (op)
    {
        case "add":
