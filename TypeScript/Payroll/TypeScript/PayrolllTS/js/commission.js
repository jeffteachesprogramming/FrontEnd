...
function calculateCommissionGrossPay(sales, percentage) {
    const gross = sales * percentage;
    const message = `${sales.toFixed(2)} sales amount x ${(percentage * 100).toFixed(0)}% commission percentage`;
...
function validateAndCalculateCommission() {
    ...
    const salesResult = validateNumericInput(salesAmountInput, "Sales Amount", MINSALES, MAXSALES);
    ...
    const pctResult = validateNumericInput(commissionPercentageInput, "Commission Percentage", MINPCT, MAXPCT);
    //  If invalid, return with error message
    if (!pctResult.valid) {
